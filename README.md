...
steps:
  ...
  # Install the `buf` CLI
  - uses: bufbuild/buf-setup-action@v1.47.1
...
|:---------------|:---------------------------------------------------|:-------------------|
| `version`      | The version of the [`buf` CLI][buf-cli] to install | [`v1.47.1`][version] |
...
steps:
  - uses: actions/checkout@v2
  # Installs version 1.47.1
  - uses: bufbuild/buf-setup-action@v1.47.1
    with:
      version: 1.47.1
  # Should output 1.47.1
  - run: buf --version
```
...
[version]: https://github.com/bufbuild/buf/releases/tag/v1.47.1
